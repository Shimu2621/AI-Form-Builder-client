...
} from "@/components/Skeleton/LoadingSkeleton";
...
const Billing = () => {
  ...
  // Track copied Stripe ID for UI feedback
  ...
  const getPlanBadge = (planType: string) => {
    ...
    return (
      <Badge
        className={`${
          planColors[planType as keyof typeof planColors] || planColors.basic
        } text-xs`}
      >
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription className="text-sm">
            ...
            <Button
              variant="outline"
              size="sm"
              className="ml-2 bg-transparent text-xs"
              onClick={() => refetchUser()}
            >
              Retry
            </Button>
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
              <Avatar className="h-12 w-12 sm:h-16 sm:w-16 mx-auto sm:mx-0">
                <AvatarImage
                  src={user.data.photo || "/placeholder.svg"}
                  alt={user.data.name}
                />
